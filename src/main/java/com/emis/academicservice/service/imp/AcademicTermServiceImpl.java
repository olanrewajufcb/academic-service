...
import com.emis.academicservice.exception.AlreadyExistsException;
...
public class AcademicTermServiceImpl implements AcademicTermService {
    ...
    public Mono<AcademicTermResponse> createAcademicTerm(CreateAcademicTermRequest request, String schoolCode, String requestId) {
        ...
        return schoolCacheService.getSchoolIdByCode(schoolCode)
                ...
                .doOnError(throwable -> log.error("Error creating academic term: {}", throwable.getMessage()))
                .onErrorMap(DataIntegrityViolationException.class, ex -> {
                    log.error("[{}] Error creating academic term: ", requestId, ex);
                    return  new AlreadyExistsException(
                            String.format("Academic term '%s' already exists for school '%s'",
